...
function updateGraph(brother)
{
    ...
    brotherData = brotherHeirarchy.descendants();//Gets array of all brothers
    flipData(brotherData);//Flips to Horizontal graph
    brotherLinks = brotherHeirarchy.links();//Gets array of all links from brothers

    drawLinks();
    drawNodes();//Will draw nodes/links with current data and remove any not found in the heirarchy
}
        ...
        (
            ...
            exit => exit
                .call(exit => exit
                    .transition()
                    .duration(duration)
                    .attr('transform', brother =>
                        {
                            return `translate(${brother.x},${brother.y})`;
                        }
                    )
                    .remove()///Will translate and remove all extra nodes
                )   
